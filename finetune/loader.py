...
class MoleculeDataset(InMemoryDataset):
    ...
    def get(self, idx):
        ...
        # n = data.num_nodes
        # s = torch.arange(n)
        # adj = torch.vstack((s.repeat_interleave(n), s.repeat(n)))
        # data.edge_index=adj
    ...
    def process(self):
        ...
        if self.dataset == 'zinc_standard_agent':
            ...
            with open('../pretrain_PI/PI/'+self.dataset+'_atom_Pi.pkl', 'rb') as f:
                topo_feature1 = pickle.load(f)
            with open('../pretrain_PI/PI/'+self.dataset+'_degree_Pi.pkl', 'rb') as f:
                topo_feature2 = pickle.load(f)
            with open('../pretrain_PI/PI/'+self.dataset+'_hks0.1_Pi.pkl', 'rb') as f:
            ...
            for i in range(len(smiles_list)):
                ...
                # the dataset
                data.y = torch.tensor(labels[i, :])
                if(i >= len(topo_feature1) or len(topo_feature1[i]) == 3):
                    data.PI = torch.zeros(150)
                else:
