...
def scaffold_split(dataset, smiles_list, task_idx=None, null_value=0,
    ...
    if not return_smiles:
        ...
        return train_dataset, valid_dataset, test_dataset, (train_smiles,
                                                            ...
                                                            test_smiles)
import random
def scaffold_split_subset(dataset, smiles_list, task_idx=None, null_value=0,
                   frac_train=0.8, frac_valid=0.1, frac_test=0.1,
                   return_smiles=False, threshold=10, split_seed=0):
    """
    Adapted from  https://github.com/deepchem/deepchem/blob/master/deepchem/splits/splitters.py
    Split dataset by Bemis-Murcko scaffolds
    This function can also ignore examples containing null values for a
    ...
    # print(train_idx)
    # n = len(torch.tensor(train_idx)) // 10
    start = (split_seed+0)*threshold
    end = (split_seed+1)*threshold
    if(end>= len(torch.tensor(train_idx))):
        random.shuffle(train_idx)
        start = len(torch.tensor(train_idx))-threshold
        end = len(torch.tensor(train_idx))
    print(start,end)
    train_dataset = dataset[torch.tensor(train_idx)[start:end]]
    valid_dataset = dataset[torch.tensor(valid_idx)]
    test_dataset = dataset[torch.tensor(test_idx)]
