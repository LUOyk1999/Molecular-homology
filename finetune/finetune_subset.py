...
def sigmoid(x):
    ...
    return torch.nn.Sigmoid()(x).numpy()
...
def main():
    ...
    if args.split == "scaffold":
        ...
        train_dataset, valid_dataset, test_dataset, smiles = scaffold_split_subset(dataset, smiles_list, null_value=0, frac_train=0.8,frac_valid=0.1, frac_test=0.1, return_smiles=True, threshold=args.threshold, split_seed=args.split_seed)
        print("scaffold")
    ...
    # test_smiles = smiles[2]
    # dataset = 'zinc_standard_agent_init'
    ...
    if not args.input_model_file == "":
        ...
        if not os.path.exists(fname):
            os.makedirs(fname)
        print_to_file(fname+'/'+args.input_model_file.split('/')[-1] + '.txt', args.dataset + ",%d,%.3f,%.3f" % (args.split_seed, Val, test))
    else:
        fname = 'result.txt'
        print_to_file(fname, args.dataset + ",%d,%.3f,%.3f" % (args.split_seed, Val, test))
