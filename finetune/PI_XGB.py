...
def main():
    ...
    for max_depth_ in range (2, 10, 1):
        for learning_rate_ in [0.05, 0.01, 0.1]:
            model = XGBClassifier(learning_rate=learning_rate_, n_estimators=60, objective='binary:logistic',
                        silent=True, max_depth=max_depth_, nthread=4, scale_pos_weight=2)
            from sklearn.multioutput import MultiOutputClassifier
            multi_clf = MultiOutputClassifier(model, n_jobs=-1)
    ...
    print_to_file("XGB.txt", best_test)
