...
def train(args, loader, model, optimizer, device, gamma_joao):
    ...
    # for step, batch in enumerate(loader, desc="Iteration"):
        ...
        PI = _.PI.reshape(batch1.id.shape[0],-1)
        
        x0 = model.forward_cl(_.x, _.edge_index, _.edge_attr, _.batch)
    ...
    b = aug_prob + beta * (loss_aug - gamma * (aug_prob - 1/5))
...
def main():
    ...
    #set up dataset
    dataset = MoleculeDataset_graphcl("../finetune/dataset/" + args.dataset, dataset=args.dataset, topo_features=dict_save)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        torch.save(model.gnn.state_dict(), "./joao_TDL" + ".pth")
