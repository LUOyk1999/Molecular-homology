...
def perturb_filter_function(g, filtration_val):
    simplex_filter = {}
    ee = 1e-6
    max_filter = 101
    for node in g.nodes():
        temp = {}
        temp['old'] = filtration_val[node]
        temp['new'] = filtration_val[node]
    ...
    for simplex in simplices:
        if len(simplex[0]) == 1:
            ...
            if parent_u != parent_v:
                ...
                #u1, v1 = simplex_filter[dict_component[u]]['new'], simplex_filter[dict_component[v]]['new']
    ...
    for simplex in simplices:
        if len(simplex[0]) == 1:
            ...
            if parent_u != parent_v:
                ...
                #if simplex_filter[dict_component[small]]['old'] > simplex_filter[min_node]['old']:
                #    PD_down += [[simplex_filter[dict_component[small]]['old'], simplex_filter[min_node]['old']]]
            else:
                Pos_edges += [simplex[0]]
    #PD = PD + PD_up + [[min_value,max_value]] + PD_down + [[max_value,min_value]]
    #print(Pos_edges)
    #print(Neg_edges)
    return np.array(PD_up), np.array([[min_value,max_value]]), np.array(PD_down), Pos_edges, Neg_edges
...
def Accelerate_PD(Pos_edges, Neg_edges, simplex_filter):
    ...
    connect = list(nx.connected_components(g))
    if(len(connect)>=2):
        print(connect,len(connect))
    PD_one = []
    for g_connect in connect:
        Parent = {}
        g_nodes = g_connect
        g_temp = g.subgraph(list(g_nodes))
        Nodes = g_temp.nodes()
        root = list(Nodes)[0]
        for edge in nx.algorithms.bfs_tree(g_temp, root).in_edges:
