...
def train(args, model, proj, loader, optimizer, device):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        optimizer.zero_grad()
        NCE_loss = args.alpha * NCE_loss_intra + args.beta * NCE_loss_inter + \
                           args.gamma * NCE_loss_proto + loss_topo(graph_reps_proj, graph_reps_proj, PI)
        NCE_loss.backward()
        optimizer.step()
...
def main():
    ...
    parser.add_argument('--output_model_file', type=str, default='graphlog_TDL', help='filename to output the model')
