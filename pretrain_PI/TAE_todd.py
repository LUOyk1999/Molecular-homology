...
class GraphDataset(object):
    ...
    def __getitem__(self, index):
        ...
        PI1 = torch.cat((PI1, torch.zeros(132-PI1.shape[0])), dim=0)
        PI2 = torch.cat((PI2, torch.zeros(132-PI2.shape[0])), dim=0)
        PI3 = torch.cat((PI3, torch.zeros(132-PI3.shape[0])), dim=0)
        data.PI = torch.cat([PI1,PI2,PI3])
        if(self.filt!=-1):
            data.PI = data.PI[(self.filt)*132:(self.filt+1)*132]
        return data
...
class Topo_Model(torch.nn.Module):
    def __init__(self, hidden_dim = 300, dropout = 0.2, filt=-1):
        ...
        if filt==-1:   
            self.projection_head = nn.Sequential(nn.Linear(hidden_dim, 300), nn.ReLU(inplace=True), nn.Linear(300, 132*3))
        else:
            self.projection_head = nn.Sequential(nn.Linear(hidden_dim, 300), nn.ReLU(inplace=True), nn.Linear(300, 132))
...
def main():
    ...
    parser.add_argument('--output_model_file', type = str, default = 'tae_todd', help='filename to output the pre-trained model')
    ...
    max_=0
    dict_save=None
    with open('./PI/zinc_atom_VR.pkl', 'rb') as f:
        dict_save1 = pickle.load(f)
        for i in dict_save1:
            max_ = max(max_,np.size(i))
    with open('./PI/zinc_bond_VR.pkl', 'rb') as f:
    ...
    print(max_)
    dataset = GraphDataset(MoleculeDataset("../finetune/dataset/" + args.dataset, dataset=args.dataset), dict_save, filt=args.filt)
