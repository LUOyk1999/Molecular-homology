...
class MoleculeDataset_graphcl(MoleculeDataset):
    def __init__(self,
        ...
        super(MoleculeDataset_graphcl, self).__init__(root, transform, pre_transform, pre_filter, dataset, empty)
    ...
    def edge_pert(self, data):
        ...
        edge_attr_add_1 = torch.tensor( np.random.randint(4, size=(edge_index_add.shape[1], 1)))
        edge_attr_add_2 = torch.tensor( np.random.randint(3, size=(edge_index_add.shape[1], 1)))
    ...
    def get(self, idx):
        ...
        dict_save1,dict_save2,dict_save3 = self.topo_features
        PI1 = torch.FloatTensor(dict_save1[idx]).flatten()
        PI2 = torch.FloatTensor(dict_save2[idx]).flatten()
        PI3 = torch.FloatTensor(dict_save3[idx]).flatten()
        data.PI = torch.cat([PI1,PI2,PI3])
        ...
        if self.aug_mode == 'no_aug':
            ...
            n_aug = np.random.choice(5, 1, p=self.aug_prob)[0]
            n_aug1, n_aug2 = n_aug//5, n_aug%5
            data1 = self.augmentations[n_aug1](data1)
            data2 = self.augmentations[n_aug2](data2)
