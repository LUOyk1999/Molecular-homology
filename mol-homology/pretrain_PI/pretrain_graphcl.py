...
def main():
    ...
    #set up dataset
    dataset = MoleculeDataset_aug("../finetune/dataset/" + args.dataset, dataset=args.dataset)
    print(dataset)

    #set up model
    gnn = GNN(args.num_layer, args.emb_dim, JK = args.JK, drop_ratio = args.dropout_ratio, gnn_type = args.gnn_type)
    if not args.input_model_file == "":
        gnn.load_state_dict(torch.load(args.input_model_file + ".pth"))
    model = graphcl(gnn)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % 20 == 0:
            torch.save(gnn.state_dict(), "./graphcl_" + str(epoch) + ".pth")
