            ...
            for i in range(len(smiles_list)):
                ...
                try:
                    ...
                    if rdkit_mol != None:  # ignore invalid mol objects
                        ...
                        data.PD = torch.FloatTensor(np.array(topo_feature[0].tolist() + topo_feature[1].tolist()))
                        data.PI = torch.FloatTensor(topo_feature[2])
                        data.filt_value = torch.FloatTensor(topo_feature[3]).view(-1, 1)
                        # leading zeros
...
class MoleculeDataset_init(InMemoryDataset):
    def __init__(self,
                 root,
                 topo_features=None,
                 #data = None,
        ...
        self.dataset = dataset
        self.root = root
        super(MoleculeDataset_init, self).__init__(root, transform, pre_transform,
                                                 pre_filter)
        self.transform, self.pre_transform, self.pre_filter = transform, pre_transform, pre_filter
